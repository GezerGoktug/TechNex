...
const ProductCards = ({ type, isFavoritePage, product }) => {
    ...
    if (user?.uid) {
      ...
      const unSub = onSnapshot(userDocRef, (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          setIsFavProduct(!!favourites[product?.id]);
        }
      });
      return () => unSub();
    }
  ...
  const handleAddCart = async () => {
    if (!isLoggedIn) {
      toastNotify(types.ERROR, "You must log in to add products to the cart.");
      return;
    }
  ...
  const favProductHandle = async () => {
    ...
    if (isFavProduct) await removeFavProduct(product.id, user.uid);
    else await addFavProduct(product, user.uid);
  };

  switch (type) {
    case types.FEATURES_PRODUCT_CARD:
      return (
        <motion.div
          variants={fadeInUp}
          whileInView="open"
